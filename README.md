# HospitalApp
<h3>Demo run of The Project - </h3>
<div align="center">
<h1 [align]="center">Home Page</h1>
    <img src="screenshot/home1.png" width="80%"/>
    <img src="screenshot/home2.png" width="80%"/>
    <img src="screenshot/home3.png" width="80%"/>
    <h1 [align]="center">Patien Registration Page Page</h1><hr>
    <img src="screenshot/registration1.png" width="80%"/><br>
    <img src="screenshot/registration2.png" width="80%"/><br>
    <img src="screenshot/registration3.png" width="80%"/><br>
    <h1 [align]="center">Patient Login Page</h1>
    <img src="screenshot/login.png" width="80%"/>
    <h1 [align]="center">Book Appointment Page</h1>
    <img src="screenshot/appbook.png" width="80%"/>
    <img src="screenshot/appbook2.png" width="80%"/>
    <h1 [align]="center">Patient Dashboard</h1>
    <img src="screenshot/udash.png" width="80%"/><br>
    <h1 [align]="center">Patient Dashboard appointment Deatils</h1>
    <img src="screenshot/patientdashAppDetails.png" width="80%"/>
    <img src="screenshot/patientDashAppDEtails2.png" width="80%"/>
    <h1 [align]="center">Doctor Registration page</h1>
    <img src="screenshot/dregister.png" width="80%"/><br>
    <img src="screenshot/dreg.png" width="80%"/><br>
    <h1 [align]="center">Doctor Dashboard</h1>
    <img src="screenshot/ddash.png" width="80%"/><br>
    <h1 [align]="center">Appontment list for doctor</h1>
    <img src="screenshot/aldoctor.png" width="80%"/><br>
    <h1 [align]="center">Adding Prescription Page</h1>
    <img src="screenshot/addprescp.png" width="80%"/><br>
    <h1 [align]="center">Admin Dashboard</h1>
    <img src="screenshot/admindash.png" width="80%"/><br>
    <h1 [align]="center">Usre list for Admin</h1>
    <img src="screenshot/aluser.png" width="80%"/><br>
    <img src="screenshot/aluser2.png" width="80%"/><br>
    <h1 [align]="center">Doctor list for Admin</h1>
    <img src="screenshot/doctor list.png" width="80%"/><br>
    <h1 [align]="center">All Appointment List  for Admin</h1>
    <img src="screenshot/aladmin.png" width="80%"/><br>
    <img src="screenshot/alad2.png" width="80%"/><br>

    



    






    
</div>
